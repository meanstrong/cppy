...
public:
    CPPY_IO_StringIO(const std::string& initial_value = "", bool readable = true, bool writable = true);
...
public:
    ...
    static constexpr std::ios_base::openmode mode(const char c)
    {
        switch (c)
        {
        case 'r':
    ...
    static constexpr std::ios_base::openmode mode(Args... args)
    {
...
public:
    CPPY_IO_BytesIO(const std::vector<char>& initial_bytes = {}, bool readable = true, bool writable = true);
